...
class PlanDispatchNode:
    ...
    def __init__(self, topic_name, sub_name, name_space_list, rate, init_poses, init_rots, step_size, plan,
                 speed=0.2, rot_vel=45, dist_variance=0.1, rad_variance=0.05):

        self.dispatchers = []
        for n in name_space_list:
            print(f"making publisher: {n}/{topic_name}")
            self.dispatchers.append(rospy.Publisher(n+"/"+topic_name, Twist, queue_size=10))
...
if __name__ == "__main__":
    ...
    node = PlanDispatchNode("cmd_vel", "odom", ["tb3_1", "tb3_2"],
                            10, 0.5, [0, 0], 0.5, plan_test)
    node.execute_plan()
